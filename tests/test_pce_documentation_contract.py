...
def test_every_test_path_named_in_the_traceability_manifest_exists():
    ...
    assert named_files, "no test file names found in the traceability manifest -- did its format change?"
    missing = [name for name in named_files if not (REPO_ROOT / "tests" / name).exists()]
    assert not missing, f"traceability manifest names test file(s) that do not exist: {missing}"
...
def test_readme_does_not_announce_an_unbuilt_generator_cli_or_adapter():
    ...
    for phrase in ("pce-assess", "command-line assessment tool", "tracking/sensor-fusion adapter", "end-to-end demonstration"):
...
def test_readme_states_no_generator_exists_yet():
    text = _read(README).lower()
    assert "no certificate generator" in text or "no generator" in text
